...
AREAS = sorted([
    ...
    # 2026-08 加：研究改用細區粒度之後，呢批地區名先出現。
    # AREAS 唔齊會令 variants() 剝唔到地區前綴，地址退化階梯直接失效。
    '太子','大角咀','田灣','啟德','京士柏','石門','火炭','大水坑','坑口','寶琳',
    '杏花邨','天后','大坑','跑馬地','薄扶林','鴨脷洲','愛民','鶴園'
...
def district_of_addr(addr):
    ...
    A2D = {'尖沙咀':'油尖旺區','旺角':'油尖旺區','油麻地':'油尖旺區','佐敦':'油尖旺區','太子':'油尖旺區',
      ...
      '太子':'油尖旺區','大角咀':'油尖旺區','京士柏':'油尖旺區',
      '美孚':'深水埗區','鶴園':'九龍城區','牛頭角':'觀塘區',
      '慈雲山':'黃大仙區','樂富':'黃大仙區',
      '天后':'灣仔區','大坑':'灣仔區','跑馬地':'灣仔區',
      '杏花邨':'東區','薄扶林':'南區','鴨脷洲':'南區',
      '大水坑':'沙田區','坑口':'西貢區','寶琳':'西貢區'}
    for a in AREAS:
        if a in addr:
            return A2D.get(a)
    return None
